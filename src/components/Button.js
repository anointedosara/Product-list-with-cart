...
function Button({ id }) {
  ...
  return (
    <div className="absolute flex items-center justify-center -top-12 w-full">
      {!isAdding ? (
        <button onClick={handleShowAdding} className='flex items-center justify-center text-[12px] font-bold w-[130px] cursor-pointer bg-white py-2 px-2 ring-1 ring-rose-500 rounded-4xl hover:ring-red hover:text-red transition-all'><Image className='w-4 mr-2' src="icon-add-to-cart.svg" alt="" width={1000} height={1000} /> Add to Cart</button>
      ) : (
        <div className='flex items-center justify-between text-[12px] w-[130px] font-bold cursor-pointer bg-red py-2 px-2 rounded-4xl'>
